          ...
          .then((res) => {
            ...
            res.data.forEach( async element => {
                if (element.name === "humid"){
                  ...
                  instance.time = moment(element.updated_at).tz("Asia/Ho_Chi_Minh").format("HH:mm:ss")
                  instance.name = element.name
                  MoistModel.findOne({time: instance.time}).lean().then(async data => {
                    // console.log(data)
                    if (data === null) {
                      await instance.save()
                      await MoistModel.find().sort({_id: -1}).limit(5).lean().then(data => {
                        ws.send(JSON.stringify(data))
                        // console.log(data)
                      })
                    }
                  ...
                  instance.time = moment(element.updated_at).tz("Asia/Ho_Chi_Minh").format("HH:mm:ss")
                  instance.name = element.name
                  TempModel.findOne({time: instance.time}).lean().then(async data => {
                    // console.log(data)
                    if (data === null) {
                      await instance.save()
                      await TempModel.find().sort({_id: -1}).limit(5).lean().then(data => {
                        ws.send(JSON.stringify(data))
                        // console.log(data)
                      })
                    }
                  ...
                  PumpModel.findOne({time: instance.time}).lean().then(async data => {
                    // console.log(data)
                    if (data === null) {
                      await instance.save()
                      await PumpModel.find().sort({_id: -1}).limit(5).lean().then(data => {
                        ws.send(JSON.stringify(data))
...
app.get('/humidData', (req, res) => {
  HumidModel.find().sort({_id: -1}).limit(5).lean().then(data => res.send(data))
})

app.get('/moistData', (req, res) => {
    MoistModel.find().sort({_id: -1}).limit(5).lean().then(data => res.send(data))
})

app.get('/tempData', (req, res) => {
    TempModel.find().sort({_id: -1}).limit(5).lean().then(data => res.send(data))
})

app.get('/pumpData', (req, res) => {
  PumpModel.find().sort({_id: -1}).limit(5).lean().then(data => res.send(data))
})
